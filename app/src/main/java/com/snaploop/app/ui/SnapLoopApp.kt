...
private fun FaceSetupScreen(
    ...
    val cameraLauncher = rememberLauncherForActivityResult(ActivityResultContracts.TakePicturePreview()) { bitmap: Bitmap? ->
        bitmap?.let { onCapture(it.toJpeg()) }
    }
    val permissionLauncher = rememberLauncherForActivityResult(ActivityResultContracts.RequestPermission()) { granted ->
        if (granted) cameraLauncher.launch(null)
    }
    val prompts = listOf("Look straight ahead", "Turn slightly left", "Turn slightly right", "Tilt slightly", "One natural angle")
    val nextPrompt = prompts.getOrElse(captures) { "Ready to finish" }

    ScreenColumn {
        BrandHeader("Face Setup", "Capture a few guided selfies so SnapLoop can recognize you reliably in event photos.")
        GradientCard(
            title = if (captures < prompts.size) nextPrompt else "Face Setup captured",
            subtitle = "$captures of ${prompts.size} guided captures",
        )
        Text("Use good lighting, keep only your face in frame, and remove anything covering your eyes.")
        if (captures < prompts.size) {
            Button(
                onClick = {
        ...
        Button(
            ...
            modifier = Modifier.fillMaxWidth(),
            enabled = captures >= 3,
        ) { Text(if (captures < prompts.size) "Finish with $captures captures" else "Finish Face Setup") }
        if (captures > 0) {
            TextButton(onClick = onReset, modifier = Modifier.align(Alignment.CenterHorizontally)) { Text("Start Over") }
        }
        Text(
            "At least 3 valid captures are required. Five are recommended for stronger cross-angle matching.",
