...
/**
 * A row returned by MediaStore can become unreadable between query and decode, especially with
 * Android 14 selected-photo access and OEM gallery providers. This is an asset-level condition,
 * not a reason to abort an entire Event scan.
 */
class PhotoUnavailableException(
    message: String,
...
class MediaStorePhotoLibrary(private val context: Context) {
    ...
    private fun openAsset(asset: LocalPhotoAsset): InputStream = try {
        resolver.openInputStream(asset.contentUri)
            ?: throw PhotoUnavailableException("Photo ${asset.id} is no longer available")
    } catch (error: PhotoUnavailableException) {
        throw error
    } catch (error: FileNotFoundException) {
        throw PhotoUnavailableException("Photo ${asset.id} is no longer available", error)
    } catch (error: SecurityException) {
        throw PhotoUnavailableException("Photo ${asset.id} is no longer accessible", error)
    }
