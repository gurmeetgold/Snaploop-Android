...
class FirebaseBiometricConsentStore(
    ...
    suspend fun load(userId: String): BiometricConsentRecord? = try {
        val data = db.collection("users").document(userId).collection("privacy").document("biometricConsent").get().awaitResult().data ?: return null
